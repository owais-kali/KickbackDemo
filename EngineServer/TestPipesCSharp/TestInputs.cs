...
{
    public class TestInputs : IPCBase
    {
        #region Test KeyInput
        public byte[] TestKeyInput()
        {
            InputPacket inputPacket = new InputPacket(0, new InputPacket.InputObject[]
                {
                    new InputPacket.InputObject
                    (
                        0,
                        new InputPacket.InputObject.Info
                        (
                            InputPacket.InputObject.Info.InputTypeEnum.Key, true
                        ), KeyCode.S
                    ),
                    new InputPacket.InputObject
                    (
                        ...
                        new InputPacket.InputObject.Info
                        (
                            InputPacket.InputObject.Info.InputTypeEnum.Key,
                            false
                        ), KeyCode.S
                    ),
                    new InputPacket.InputObject
            ...
            byte[] inputPacketBytes = GetBinMsg(inputPacket.ToByteArray(inputPacket), BinType.Input, 1);
            return inputPacketBytes;
        }
            ...
            InputPacket inputPacket = new InputPacket(0, new InputPacket.InputObject[]
                {
                    ...
                    (
                        ...
                        new InputPacket.InputObject.Info
                        (
                            InputPacket.InputObject.Info.InputTypeEnum.Key, false
                        ), KeyCode.W
                    ),
                    new InputPacket.InputObject
                    (
        ...
        public byte[] MoveUp(int clientId = 1)
        {
            InputPacket inputPacket = new InputPacket(200, new InputPacket.InputObject[]
                {
                    new InputPacket.InputObject
                    (
                        10,
            ...
            byte[] inputPacketBytes = GetBinMsg(inputPacket.ToByteArray(inputPacket), BinType.Input, clientId);
            return inputPacketBytes;
        }
        #endregion
        ...
        public byte[] Mouseup()
        {
            InputPacket inputPacket = new InputPacket(0, new InputPacket.InputObject[]
            {
                new InputPacket.InputObject(
                    1000,
                    new InputPacket.InputObject.Info(InputPacket.InputObject.Info.InputTypeEnum.MouseMove,false),
                    KeyCode.None,0,10,600
                    )
            });
            byte[] inputPacketBytes = GetBinMsg(inputPacket.ToByteArray(inputPacket), BinType.Input, 1);
            return inputPacketBytes;
        ...
        {
            /// <summary>
            /// Packet time in ms from session start - uint32
            /// </summary>
            public uint SessionTime;

            public InputObject[] inputObjectArray;

            public InputPacket(uint sessionTime, InputObject[] inputObjectArray)
            {
                SessionTime = sessionTime;
                this.inputObjectArray = inputObjectArray;
            }

            public InputPacket(byte[] msgData)
            {
                SessionTime = BitConverter.ToUInt32(msgData, 0);
                inputObjectArray = ByteToInputObjectArray().ToArray();

                List<InputObject> ByteToInputObjectArray()
                {
                    List<InputObject> tempInputObject = new List<InputObject>();
                    int msgDataCounter = 4;

                    InputObject inputObject;
                    while (msgDataCounter < msgData.Length)
                    {
                        inputObject = new InputObject
                        {
                            deltaTime = BitConverter.ToUInt16(msgData, msgDataCounter),
                            _mouseX = 0, _mouseY = 0,
                            info = new InputObject.Info(msgData[msgDataCounter + 2]),
                            keycode = KeyCode.Alpha0
                        };
                        msgDataCounter += 3;

                        switch (inputObject.info.InputType)
                        {
                            case InputObject.Info.InputTypeEnum.Key:
                                inputObject.keycode = (KeyCode) BitConverter.ToUInt16(msgData, msgDataCounter);
                                //inputObject.keycode = (KeyCode) msgData[msgDataCounter];
                                msgDataCounter += 2;
                                break;
                            case InputObject.Info.InputTypeEnum.MouseScroll:
                                inputObject.scrollCount = msgData[msgDataCounter];
                                msgDataCounter++;
                                break;
                            case InputObject.Info.InputTypeEnum.MouseMove:
                                inputObject._mouseX = BitConverter.ToUInt16(msgData, msgDataCounter);
                                inputObject._mouseY = BitConverter.ToUInt16(msgData, msgDataCounter + 2);
                                msgDataCounter += 4;
                                break;
                        }

                        tempInputObject.Add(inputObject);
                    }
            ...
            public byte[] ToByteArray(InputPacket inputPacket)
            {
                int InputObjectByteArrayLen = 4;

                for (int i = 0; i < inputPacket.inputObjectArray.Length; i++)
                {
                    InputObject inputObject = inputPacket.inputObjectArray[i];
                    InputObjectByteArrayLen += 3;
                    switch (inputObject.info.InputType)
                    {
                        case InputObject.Info.InputTypeEnum.Key:
                            InputObjectByteArrayLen += 2;
                            break;
                        case InputObject.Info.InputTypeEnum.MouseScroll:
                            InputObjectByteArrayLen += 1;
                            break;
                        case InputObject.Info.InputTypeEnum.MouseMove:
                            InputObjectByteArrayLen += 4;
                            break;
                    }
                }
            
                byte[] inputObjectByteArray = new byte[InputObjectByteArrayLen];

                //InputObjectByteArray Add Session Time
                BitConverter.GetBytes(inputPacket.SessionTime).CopyTo(inputObjectByteArray, 0);

                int msgCounter = 4;
                for (int i = 0; i < inputPacket.inputObjectArray.Length; i++)
                {
                    InputObject inputObject = inputPacket.inputObjectArray[i];
                    //Add Packet Time uint16
                    Array.Copy(BitConverter.GetBytes(inputObject.deltaTime), 0,inputObjectByteArray,msgCounter, 2);
                    msgCounter += 2;

                    //Add info 1 Byte
                    inputObjectByteArray[msgCounter] = inputObject.info;
                    msgCounter++;
                    ...
                    {
                        case InputObject.Info.InputTypeEnum.Key:
                            Array.Copy(BitConverter.GetBytes((ushort) inputObject.keycode), 0, inputObjectByteArray,
                                msgCounter, 2);
                            msgCounter += 2;
                            break;
                        case InputObject.Info.InputTypeEnum.MouseScroll:
                            inputObjectByteArray[msgCounter] = inputObject.scrollCount;
                            msgCounter++;
                            break;
                        case InputObject.Info.InputTypeEnum.MouseMove:
                            Array.Copy(BitConverter.GetBytes((ushort) inputObject._mouseX), 0, inputObjectByteArray,
                                msgCounter, 2);
                            msgCounter += 2;
                            Array.Copy(BitConverter.GetBytes((ushort) inputObject._mouseY), 0, inputObjectByteArray,
                                msgCounter, 2);
                            msgCounter += 2;
                            break;
                    }
            ...
            {
                /// <summary>
                /// Input time in ms relative to packet time - uint16
                /// </summary>
                public ushort deltaTime;

                /// <summary>
                /// Input information - uint8
                /// </summary>
                public Info info;

                /// <summary>
                /// Key code - uint16 (Context dependent extra data)
                /// </summary>
                public KeyCode keycode;

                /// <summary>
                /// Scroll magnitude - int8 (Context dependent extra data)
                /// </summary>
                public byte scrollCount;

                /// <summary>
                /// Cursor position (X,Y) - uint16 + uint16 (Context dependent extra data)
                /// </summary>
                public ushort _mouseX, _mouseY;

                public InputObject(ushort deltaTime, Info info, KeyCode keycode, byte scrollCount=0, ushort mouseX=0,
                    ushort mouseY=0)
                {
                ...
                public struct Info
                {
                    ...
                    //public MouseBtnEnum MouseBtn;
                    public bool IsKeyDown;

                    public Info(InputTypeEnum inputType, bool isKeyDown)
                    {
                        InputType = inputType;
                        IsKeyDown = isKeyDown;
                    }
                    ...
                    public Info(byte b)
                    {
                        ...
                        {
                            case 0:
                                InputType = InputTypeEnum.Key;
                                break;
                            case 2:
                                InputType = InputTypeEnum.MouseScroll;
                                break;
                            default:
                                InputType = InputTypeEnum.MouseMove;
                                break;
                        }
                        
                        IsKeyDown = false;

                        if (InputType == InputTypeEnum.Key)
                        {
                            IsKeyDown = (b & 4) == 4;
                        }
                    }

                    public static implicit operator byte(Info i)
                    {
                        byte b = (byte) i.InputType;
                        if (i.IsKeyDown) b += 0b00000100;
                        return b;
                    }
                    public static explicit operator Info(byte b) => new Info(b);
                }
            }
        }
    }
